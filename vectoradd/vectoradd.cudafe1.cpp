...
# 40 "/usr/lib/gcc/x86_64-linux-gnu/6/include/stdarg.h" 3
...
# 81
{ (__cudaPushCallConfiguration(gridSize, blockSize, 0, 0)) ? (void)0 : vecAdd(d_a, d_b, d_c, n); cudaError err = cudaGetLastError(); if ((cudaSuccess) != err) { fprintf(stderr, "Cuda error in function \'%s\' file \'%s\' in line %i : %s.\n", "(vecAdd<<<gridSize, blockSize,0,0>>>(d_a, d_b, d_c, n))", "vectoradd.cu", 81, cudaGetErrorString(err)); 
# 81 "vectoradd.cu"
fflush(stderr); 
# 81 "vectoradd.cu"
exit(1); 
...
#define _NV_ANON_NAMESPACE _GLOBAL__N__17_vectoradd_cpp1_ii_6cd02eb8
# 1 "vectoradd.cudafe1.stub.c"
#include "vectoradd.cudafe1.stub.c"
# 1 "vectoradd.cudafe1.stub.c"
